"""Session helper for e-shuushuu.net.

The site is a SvelteKit front end over a JSON API at ``/api/v1/``. The pyskeb prototype scraped
HTML pages fifteen images at a time; that markup is gone, and the API replaces it.

No credentials: the API answers anonymously and the fields that would differ for a logged-in
caller (``is_favorited``, ``user_rating``) are of no use to an index anyway.

Getting in from CI
==================

Untested from a runner when this was written, hence the proxy fallback. aibooru is the cautionary
case: it answers a residential address on every fingerprint and a GitHub runner on none, and the
cause only became visible once the response body was logged.

Direct is tried first because the pool is billed per request; the pool's exits are sampled a few at
a time because they rotate, and on aibooru the first exit was challenged while the second was not.
"""
...
__site_url__ = 'https://e-shuushuu.net'
...
class EShuushuuError(Exception):
    """Raised when no session can be established."""


def get_eshuushuu_session(impersonate: Optional[str] = None, timeout: float = DEFAULT_TIMEOUT,
    ...
    :raises EShuushuuError: When no fingerprint can get through.
    """
    ...
    if session is not None:
        return session
    if not proxy_pool:
        raise EShuushuuError(f'Could not get a usable session directly and no proxy is configured; '
    ...
    if brd_api_key:
        try:
            if not ensure_proxy_access(proxy_pool, api_key=brd_api_key, zone=brd_zone):
                raise EShuushuuError('Proxy pool is not usable from this host.')
        except BrightDataError as err:
            raise EShuushuuError(f'Proxy pool unusable - {err}') from err
    ...
    if session is not None:
        return session
    raise EShuushuuError(f'Could not get a usable session, direct or proxied; '
                     f'last attempt - {_LAST_FAILURE[0]}')
...
def _walk_ladder(ladder, timeout, username, api_key, proxy):
    ...
    for chosen in ladder:
        ...
        try:
            resp = session.get(f'{__site_url__}/api/v1/images', params={'limit': '1'})
            if resp.status_code != 200:
                ...
                last = (f'{chosen}: HTTP {resp.status_code} {marks} hints={hints} '
                        f'body={body[:400]!r}')
                logging.warning(f'EShuushuu session attempt failed ({route}) - {last}.')
                continue
            resp.json()
        except Exception as err:
            ...
            logging.warning(f'EShuushuu session attempt failed ({route}) - {last}.')
            continue
        logging.info(f'EShuushuu session ready with fingerprint {chosen!r} ({route})'
