...
def repo_squash():
    ...
    remote_repo = os.environ.get('REMOTE_REPOSITORY_DB_N')
    ...
    remote_repo = os.environ.get('REMOTE_REPOSITORY_DB_N_PUBLIC')
    if len(hf_client.list_repo_commits(repo_id=remote_repo, repo_type='dataset')) >= 500:
        hf_client.super_squash_history(repo_id=remote_repo, repo_type='dataset', commit_message='Squashed!')
    remote_repo = os.environ.get('REMOTE_REPOSITORY_DB_N_PUBLIC_4M')
    if len(hf_client.list_repo_commits(repo_id=remote_repo, repo_type='dataset')) >= 500:
        hf_client.super_squash_history(repo_id=remote_repo, repo_type='dataset', commit_message='Squashed!')
